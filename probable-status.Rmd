...
sp_data <- left_join(raw_sp_data, select(cu_metadata, CUID, du_number, DU_name,gen_length), 
                     by="CUID") %>%
  drop_na(du_number, Spawner.Abundance) %>%
  mutate(du_cu=paste(du_number, DU_name)) %>%
  relocate(du_cu, 1) %>%
  arrange(du_cu, Year) %>%
  ...
  for(j in unique(sp_data_region$Species)){
    ...
    for(k in 1:pages){
      ...
      p <- ggplot(data = sub_data_2, aes(x=Year, y=Spawner.Abundance/1000)) +
        ...
        theme_bw() +
        theme(strip.background=element_blank(), axis.text.x=element_text(angle=45), strip.text = element_text(size = 4))
      print(p)
      ...
      p_log <- ggplot(data=sub_data_2, aes(x=Year, y=logSp)) +
        ...
        theme_bw() +
        theme(strip.background=element_blank(), axis.text.x=element_text(angle=45), strip.text = element_text(size = 7))
      print(p_log)
    }
  }else{
