...
#check for dup here
sp_data |>
  select(du_number, Region, Year, Spawner.Abundance) |> #what distinguishes du*years    
  #select(CUID, Region, Year, Spawner.Abundance) |> #what distinguishes du*years
  distinct() |>
  nrow() == nrow(sp_data) #should be TRUE! #ID these dups and see

#ID the dups
bad_dups <- sp_data |>
  group_by(du_number, Region, Year, Spawner.Abundance) |>
  filter(n()>1)
