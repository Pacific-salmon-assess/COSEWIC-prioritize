...
This document summarizes estimates of change in mature individuals over time for Pacific salmon Designatable Units (DUs) and associated probable COSEWIC status designations. These estimates are being used to help COSEWIC's Marine Fishes sub-committee identify high priority DUs to recommend for formal status assessments. 

The specific COSEWIC criteria that are used for this rapid status assessment include A2 (Endangered = greater than 50% decline over most recent three generations; Threatened = greater than 30% (and less than 50%) decline over most recent three generations) and D1 (Endangered = fewer than 250 mature individual in recent generation; Threatened = fewer than 1000 (but greater than 250) mature individual in recent generation).

Data sources are detailed [here](https://github.com/Pacific-salmon-assess/COSEWIC-prioritize#data-sources) and all code to reproduce the analyses in this document is [here](https://github.com/Pacific-salmon-assess/COSEWIC-prioritize). These materials are solely intended to support the Marine Fishes sub-committee of COSEWIC, and may not be appropriate for other applications.
...
for(i in regions_ordered){
  ...
  for(j in unique(sp_data_region$Species)){
    ...
    p <- ggplot(data=sub_data, aes(x=Year, y=Spawner.Abundance/1000)) +
      geom_line(color="grey") +
      geom_point() +
      stat_smooth(formula=log(y)~x, method="lm", color="black", lty="dashed", se=FALSE) +
      stat_smooth(data=sub_last3, formula=log(y)~x, method="lm", color="red", 
                  na.rm=TRUE) +
      #stat_smooth(formula=log(y)~x, method="lm", color="black", lty="dashed", se=FALSE) +
      #stat_smooth(data=sub_last3, formula=log(y)~x, method="lm", color="red", 
      #            na.rm=TRUE) +
      #ggh4x::facet_wrap2(vars(du_cu), nrow=rows, ncol=cols, trim_blank=FALSE, 
      #                   scales="free_y") +
      facet_wrap(vars(du_cu), scales = "free_y") +
