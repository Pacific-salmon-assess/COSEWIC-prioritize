...
nrow(distinct(raw_sp_data, CUID))
...
cu_metadata <- read.csv("./data/CU_Metadata_20230828.csv",  
  ...
  rename(CUID=cuid) |>
  distinct()
#THERE ARE STILL DUPLICATES in cu_metadata that need fixing so we don't get the warning below

# join, dump pops with NA DUs ----
sp_data <- left_join(raw_sp_data, select(cu_metadata, Region, CUID, du_number, gen_length, 
                                         Sites, cu_enh_rank, COSEWIC_status_2023, spp), 
                     by="CUID") %>%
