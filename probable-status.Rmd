...
```{r wrangle}
# read in data ----
raw_sp_data <- read.csv("./data/CU_Spawner_Abund_20240808.csv", 
                        header=TRUE) |>
  mutate(logSp=log(Spawner.Abundance))
nrow(distinct(raw_sp_data, CUID))#, Species, Region))
...
cu_metadata <- read.csv("./data/CU_Metadata_20240808.csv",  
...
sp_data <- left_join(raw_sp_data, select(cu_metadata, Region, CUID, du_number, gen_length, 
                                         Sites, cu_enh_rank, COSEWIC_status, spp), 
                     by="CUID") |>
...
designations <- rbind(assessed,not_assessed)|>
  mutate(species = spp) |>
  select(region_full, du_number,species, gen_length, COSEWIC_status,per_change_recent, 
