...
#A TRY COPYING THINGS I KNOW BETTER!!
...
for(i in unique(sp_data$Region)){
  ...
  #HERE IS WHERE I WOULD WANT TO NEST A REGION HEADER IF I COULD!
  ...
  for(j in unique(sp_data_region$Species)){
    ...
    #AND HERE I'D NEST A SPECIES HEADER!
...
#WAS TRYING TO USE EARLIER OBJECTS BUT HITTING A WALL! 

#turn it off for now - might revisit?
if(FALSE){
  max_facets <- 20 #how many facets (i.e. panes) per plot?
rows <- 4 #how many rows/cols? note needs to be multiple of facets. 
cols <- 5
...
sp_data <- sp_data %>%
  ungroup() %>%
  mutate(log_mat_ind = log(mat_ind))
...
for(i in unique(sp_data$Species)){
  ...
  for(j in unique(sp_data_species$Region)){
    sub_data <- filter(sp_data_species, Region==j) %>%
      complete(Year, Species, Region, du_number) #explicitly include NA so line draws right
    sub_last3 <- filter(last3_gens, Species==i, Region==j)
    n_pops <- length(unique(sub_data$du_number))
    if(n_pops>max_facets){
      pages <- ceiling(n_pops/max_facets)
    for(k in 1:pages){
      ...
      sub_last3_2 <- filter(sub_last3, du_number %in% 
                                unique(sub_last3$du_number)[((k-1)*max_facets)+1:(max_facets*k)])
      ...
      p_log <- ggplot(data=sub_data_2, aes(x=Year, y=log_mat_ind)) +
        geom_line(color="grey") +
        geom_point() +
        stat_smooth(method="lm", formula=y~x, color="black", lty="dashed", se=FALSE) +
        stat_smooth(data = sub_last3_2, formula=y~x, method="lm", color="red", se=FALSE, 
    ...
    p_log <- ggplot(data=sub_data, aes(x=Year, y=log_mat_ind)) +
      geom_line(color="grey") +
      geom_point() +
      stat_smooth(formula=y~x, method="lm", color="black", lty="dashed", se=FALSE) +
      stat_smooth(data=sub_last3, formula=y~x, method="lm", color="red", se=FALSE, 
