...
raw_sp_data <- read.csv("https://raw.githubusercontent.com/hertzPSF/COSEWIC-compilation/main/Output/CU_Spawner_Abund_20240808.csv", 
                        header=TRUE) |>
  mutate(logSp=log(Spawner.Abundance), 
         Species = ifelse(grepl("Pink", Species), "Pink", Species)) #omit even and odd groupings
...
cu_metadata <- read.csv("https://raw.githubusercontent.com/hertzPSF/COSEWIC-compilation/main/Output/CU_Metadata_20240808.csv",  
...
if(FALSE){
  nrow(distinct(cu_metadata, cuid))
  nrow(distinct(cu_metadata, cuid, cuname.x))
  nrow(distinct(cu_metadata, cuid, du_number))
  cu_check <- filter(cu_metadata, is.na(gen_length))
  
  cu_metadata_summary <- cu_metadata |> 
    group_by(CUID) |>
    summarise(count = n()) |>
    filter(count > 1) |>
    arrange(CUID)
  
  write.csv(cu_metadata_summary, "C:/Users/GLASERD/Desktop/duplicate_CUID.csv", row.names = F)
}

cu_metadata <- cu_metadata |>
  filter(!(Region == "Skeena" & cuid %in% dup_CUs$cuid)) #removing duplicate Skeena metadata
...
cu_metadata <- cu_metadata |>
  ...
  select(CUID, Region, Species, gen_length, COSEWIC_status, cu_enh_rank) |>
  distinct() #just going distinct on all levels for now... 

#nrow(cu_metadata) - nrow(distinct(cu_metadata, CUID)) #how many rows have multiple CUID now?

# join, dump pops with NA DUs ----
sp_data <- left_join(raw_sp_data, cu_metadata, 
                     by=c("CUID", "Species", "Region")) |>
  drop_na(Spawner.Abundance, gen_length) 
...
last3_gens_filtered <- filter(last3_gens, !(CUID %in% deficient_recent))
...
sp_data_filtered <- filter(sp_data, !(CUID %in% deficient_historical))
```
...
for(i in unique(sp_data_filtered$CUID)){
  sub_raw <- filter(sp_data, CUID==i)
  sub_data <- filter(sp_data_filtered, CUID==i)
  sub_last3 <- filter(last3_gens_filtered, CUID==i)
  if(nrow(sub_data)==0){
    slope_all <- NA
  } else{
    if(nrow(sub_last3)==0){
      ...
      model_all <- lm(log(sub_data$Spawner.Abundance)~sub_data$Year)
      model_recent <- lm(log(sub_last3$Spawner.Abundance)~sub_last3$Year)
  ...
  output <- data.frame(CUID=i, Region = unique(sub_raw$Region), per_change_recent, per_change_all, recent_abundance, last_year_monitored)
...
master_desg <- left_join(cu_metadata, select(prob_desg, Region, CUID, per_change_recent, per_change_all, recent_abundance, last_year_monitored, prob_designation_recent,prob_designation_all), 
                         by="CUID") |>
...
designations <- rbind(assessed,not_assessed)|>
  select(region_full, CUID, Species, gen_length, COSEWIC_status,per_change_recent, 
...
for(i in 1:length(designations$CUID)){
...
ggplot(count_bins_status, aes(x = Species, y = count)) + 
...
sp_data <- left_join(raw_sp_data, select(cu_metadata, CUID, Region, gen_length), 
                     by="CUID") |> 
  drop_na(CUID, Spawner.Abundance) |>
  arrange(CUID, Year) |>
  mutate(Region = case_when(Region.x == "HG" ~ "Haida Gwaii",
                            Region.x == "CC" ~ "Central Coast",
                            Region.x == "VIMI" ~ "Van. Isl. Main. Inl.", 
                            TRUE ~ Region.x))
}
 
if(FALSE){ #checking things that plot bad
check.VI.CK <- filter(sp_data, du_cu == "CK21 ECVI-Ocean-Fall")
}
...
for(i in regions_ordered){
  ...
  for(j in unique(sp_data_region$Species)){
    sub_data <- filter(sp_data_region, Species==j) |>
      complete(Year, Species, Region, CUID) #explicitly include NA so line draws right
    
    cat('\n')  
    cat("### ", j, "\n") #nested species within region header
    
    sub_last3 <- filter(last3_gens, Region==i, Species==j)
    n_pops <- length(unique(sub_data$CUID))
    if( n_pops>max_facets){
      pages <- ceiling(n_pops/max_facets)
      for(k in 1:pages){
        sub_data_2 <- filter(sub_data, CUID %in% 
                               unique(sub_data$CUID)[((k-1)*max_facets)+1:(max_facets*k)])
        
        sub_last3_2 <- filter(sub_last3, CUID %in% 
                                unique(sub_last3$CUID)[((k-1)*max_facets)+1:(max_facets*k)])
      ...
      p <- ggplot(data=sub_data_2, aes(x=Year, y=Spawner.Abundance/1000)) +
        ...
        ggh4x::facet_wrap2(vars(CUID), nrow=rows, ncol=cols, trim_blank=FALSE, 
                           scales="free_y") +
      ...
      p <- ggplot(data=sub_data, aes(x=Year, y=Spawner.Abundance/1000)) +
        ...
        theme_bw() +
        theme(strip.background=element_blank(), axis.text.x=element_text(angle=45), vjust = 0.5, 
              strip.text = element_text(size = 5), strip.text.x = element_text(size=10))
      print(p)
    }
    cat('\n')  
  }
...
if(FALSE){
for(i in unique(last3_gens_filtered$CUID)){
  sub_data <- filter(last3_gens_filtered, CUID == i)
  ...
  slope_posterior <- bind_rows(data.frame(CUID =i, Region = unique(sub_data$Region),
