...
raw_sp_data <- read.csv("compilation/COSEWIC-compilation-main/Output/CU_Spawner_Abund_20220802.csv",
                        header=TRUE) %>%
         mutate(logSp=log(Spawner.Abundance))

cu_metadata <- read.csv("compilation/COSEWIC-compilation-main/Output/CU_Metadata_20220802.csv",
...
```{r abundance plots, echo = FALSE, warning = FALSE}
...
## Population decline  
### Basic linear models  
Now we'll run basic linear models on ln(spawner abundance) for the whole time series and
for the last 3 generations. We add some if statements to account for which du_cus made it through data filters above.   
```{r}
summary_table <- NULL
...
**note the MetricsCOSEWIC package is still in development**  
