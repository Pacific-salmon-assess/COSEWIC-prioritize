...
First some housekeeping. You can use devtools to download the [MetricsCOSEWIC package](https://github.com/SOLV-Code/MetricsCOSEWIC) from GitHub. We'll wrangle the data into the format this package wants as well. We'll feed it all the data, not the data deficient stuff we filtered out earlier.  
...
for(i in unique(sp_data$du_cu)){
  sub_data <- filter(sp_data, du_cu == i)
  ...
  slope_posterior <- bind_rows(data.frame(draws = stan_mod$samples$slope, du_cu =i), 
                               slope_posterior)
...
#nest with species and region, AND do it for last 3 gens.  
for(i in unique(slope_posterior$du_cu)){
  sub_data <- filter(slope_posterior, du_cu == i)
  median <- filter(stan_summary, du_cu == i, var == "slope")$median #pluck median
  p <- ggplot(data = sub_data, aes(draws)) +
    geom_density(fill = "gray") + #make this sexier using density 
    geom_vline(xintercept = median, color="red") +
    theme_minimal() +
    labs(title = paste(i, "slope posterior"), x = "MCMC draws" )
