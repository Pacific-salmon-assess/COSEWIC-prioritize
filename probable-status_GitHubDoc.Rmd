...
The status summaries for all DUs are available for viewing or download in a large table [here](https://github.com/Pacific-salmon-assess/COSEWIC-prioritize/blob/main/output/master-prob-status/master_status.csv). A preview of the table is shown below.  
```{r, warning = FALSE}
...
#have to turn this off for now because it's a HTML widget that won't work in a github_document. ugh. 
if(FALSE){ 
...
designations2 <- designations %>%
  select(Region, Species, DU,'% Change (recent)', 'COSEWIC status', 'Probable status')

head(designations2) # a for-now workaround
```
